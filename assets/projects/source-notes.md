...
This page identifies the displayed files, saved generation records and original artwork sources.

## Displayed videos

| Study | File | Dimensions / frame rate | Duration / frames |
|---|---|---|---|
...
| Mimu | [Video](mimu/video.mp4) | 2160 × 2880 / 24 fps | 10 seconds / 240 |
...
The downloadable API records provide the executed seeds and generation settings. Saved UI labels and filenames may contain older values. Public workflow downloads omit local computer paths; prompts and generation parameters remain intact.

- Tide in a Cup: [Prompt](tide/prompt.txt) · [Saved workflow](tide/ui.json) · [Executed API](tide/api.json) · [Workflow image](tide/workflow.png)
- Sky Whale: [Prompt](whale/prompt.txt) · [Saved workflow](whale/ui.json) · [Executed API](whale/api.json) · [Workflow image](whale/workflow.png)
- Arcaea 7.0 story illustration: [Prompt](cg2s/prompt.txt) · [Saved workflow](cg2s/ui.json) · [Workflow image](cg2s/workflow-overview.png)
- ⊥⊬ (Sacrosanct) / Silentroom Dismantled XOR かたぎり: [Prompt](c10/prompt.txt) · [Saved workflow](c10/ui.json) · [Workflow image](c10/workflow-overview.png)
- Insight (Nullified): [Prompt](character98/prompt.txt) · [Saved workflow](character98/ui.json) · [Executed API](character98/api.json) · [Workflow image](character98/workflow.png)
- Saya (Absolution): [Prompt](character97/prompt.txt) · [Saved workflow](character97/ui.json) · [Executed API](character97/api.json) · [Workflow image](character97/workflow.png)
- Mimu: [Prompt](mimu/prompt.txt) · [Saved workflow](mimu/ui.json) · [Executed API](mimu/api.json) · [Workflow image](mimu/workflow.png)

Workflow images are offline diagrams with English labels, derived from saved workflow data. Stills and posters are frames from the displayed videos. The two process frames for each study come from the same video.
